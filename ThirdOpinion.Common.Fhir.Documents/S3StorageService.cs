...
public class S3StorageService : IS3StorageService
{
    ...
    /// <inheritdoc />
    public async Task<bool> CheckFileExistsAsync(string bucket,
        string key,
        CancellationToken cancellationToken = default)
    {
        ...
        _logger.LogDebug(
            "Checking if file exists: s3://{Bucket}/{Key} [CorrelationId: {CorrelationId}]",
        ...
        try
        {
            ...
            try
            {
                ...
                _logger.LogDebug(
                    "File existence check result: s3://{Bucket}/{Key} exists: {Exists} [CorrelationId: {CorrelationId}]",
        ...
        {
            _logger.LogError(ex,
                "Error checking file existence: s3://{Bucket}/{Key} [CorrelationId: {CorrelationId}]",
                bucket, key, correlationId);
            throw;
        }
    }
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        _logger.LogDebug(
            "Uploading file: s3://{Bucket}/{Key}, Size: {Size} bytes [CorrelationId: {CorrelationId}]",
        ...
        try
        {
            ...
            if (tags != null && tags.Count > 0) request.TagSet = tags;
            ...
            PutObjectResponse? uploadResult
                = await _s3Client.PutObjectAsync(request, cancellationToken);

            var s3Location = $"s3://{bucket}/{key}";

            _logger.LogInformation(
                "Successfully uploaded file: {S3Location}, ETag: {ETag} [CorrelationId: {CorrelationId}]",
        ...
        {
            _logger.LogError(ex,
                "Error uploading file: s3://{Bucket}/{Key} [CorrelationId: {CorrelationId}]",
                bucket, key, correlationId);
            throw;
        }
    }
    ...
    /// <inheritdoc />
    public async Task<bool> HandleForceOverwriteAsync(bool forceMode,
        string bucket,
        string key,
        CancellationToken cancellationToken = default)
    {
        if (!forceMode)
        {
            bool exists = await CheckFileExistsAsync(bucket, key, cancellationToken);
            if (exists)
            {
                string correlationId = _correlationIdProvider.GetCorrelationId();
                _logger.LogWarning(
                    "File already exists and force mode is disabled: s3://{Bucket}/{Key} [CorrelationId: {CorrelationId}]",
                    bucket, key, correlationId);
    ...
    /// <inheritdoc />
    public async Task<S3FileInfo?> GetFileInfoAsync(string bucket,
        string key,
        CancellationToken cancellationToken = default)
    {
        if (string.IsNullOrWhiteSpace(bucket))
        ...
        try
        {
            ...
            try
            {
                ...
                return new S3FileInfo
                {
                    ...
                    Metadata = metadata.Metadata.Keys.ToDictionary(k => k,
        ...
        {
            _logger.LogError(ex,
                "Error getting file info: s3://{Bucket}/{Key} [CorrelationId: {CorrelationId}]",
                bucket, key, correlationId);
            throw;
        }
    }
    ...
    /// <inheritdoc />
    public async Task<bool> DeleteFileAsync(string bucket,
        string key,
        CancellationToken cancellationToken = default)
    {
        if (string.IsNullOrWhiteSpace(bucket))
        ...
        try
        {
            ...
            _logger.LogInformation(
                "Successfully deleted file: s3://{Bucket}/{Key} [CorrelationId: {CorrelationId}]",
                bucket, key, correlationId);
        ...
        {
            _logger.LogError(ex,
                "Error deleting file: s3://{Bucket}/{Key} [CorrelationId: {CorrelationId}]",
                bucket, key, correlationId);
            return false;
        }
    }
