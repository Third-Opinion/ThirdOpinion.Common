...
public class PatientEverythingService : IPatientEverythingService
{
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        _logger.LogInformation(
            "Starting retrieval of DocumentReference resources for patient: {PatientId} [CorrelationId: {CorrelationId}]",
        ...
        try
        {
            ...
            {
                BundleData bundle
                    = await GetPatientEverythingBundleAsync(patientId, nextPageUrl,
                        cancellationToken);
                ...
                List<DocumentReferenceData> documents = bundle.GetDocumentReferences();
                allDocuments.AddRange(documents);

                _logger.LogDebug(
                    "Retrieved {Count} DocumentReference resources from page (Bundle entries: {BundleEntries}), total DocumentReferences so far: {Total} [CorrelationId: {CorrelationId}]",
            ...
            _logger.LogInformation(
                "Successfully retrieved {TotalCount} DocumentReference resources for patient: {PatientId} [CorrelationId: {CorrelationId}]",
        ...
        {
            _logger.LogError(ex,
                "Failed to retrieve DocumentReference resources for patient: {PatientId} [CorrelationId: {CorrelationId}]",
                patientId, correlationId);
            throw;
        }
    }
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        try
        {
            ...
            _logger.LogInformation(
                "Using HealthLake configuration - Region: {Region}, DatastoreId: {DatastoreId}",
                _config.Region, _config.DatastoreId);

            _logger.LogDebug(
                "Sending Patient/$everything request to: {Endpoint} [CorrelationId: {CorrelationId}]",
            ...
            HttpResponseMessage? response = await retryPolicy.ExecuteAsync(async () =>
            {
                ...
                return await _healthLakeHttpService.SendSignedRequestAsync(clonedRequest,
                    cancellationToken);
            });

            // Handle response and parse Bundle
            BundleData bundle
                = await HandleBundleResponseAsync(response, patientId, cancellationToken);

            _logger.LogDebug(
                "Successfully retrieved Bundle for patient: {PatientId}, entries: {EntryCount} [CorrelationId: {CorrelationId}]",
    ...
    private async Task<BundleData> HandleBundleResponseAsync(HttpResponseMessage response,
        string patientId,
        CancellationToken cancellationToken)
    {
        ...
        if (!response.IsSuccessStatusCode)
        {
            ...
            _logger.LogError(
                "HealthLake request failed with status: {StatusCode}, Content: {Content} [CorrelationId: {CorrelationId}]",
        ...
        try
        {
            ...
            if (bundle == null)
                throw new DocumentDownloadException(
                    "Failed to deserialize Bundle response from HealthLake");

            _logger.LogDebug(
                "Successfully parsed Bundle response: Type={Type}, Total={Total}, Entries={EntryCount} [CorrelationId: {CorrelationId}]",
        ...
        {
            _logger.LogError(ex,
                "Failed to deserialize Bundle response for patient: {PatientId} [CorrelationId: {CorrelationId}]",
                patientId, correlationId);
            throw new DocumentDownloadException("Failed to parse Bundle response from HealthLake",
                ex);
        }
    }
}