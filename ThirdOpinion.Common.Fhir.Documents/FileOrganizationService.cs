...
public class FileOrganizationService : IFileOrganizationService
{
        ...
        string contentType)
    {
        ...
        try
        {
            ...
            if (!string.IsNullOrWhiteSpace(originalFileName))
            {
                ...
                string sanitizedOriginal
                    = SanitizeFileName(Path.GetFileNameWithoutExtension(originalFileName));
                baseFileName = $"{documentRefId}-{arrayIndex}-{sanitizedOriginal}{extension}";
            }
            else
            {
                // No original filename provided, use content type info
                string typeHint = GetTypeHintFromContentType(contentType);
                baseFileName = $"{documentRefId}-{arrayIndex}-{typeHint}{extension}";
            }

            _logger.LogDebug(
                "Generated binary filename: {FileName} for patient: {PatientId}, document: {DocumentId}, index: {Index} [CorrelationId: {CorrelationId}]",
        ...
        {
            _logger.LogError(ex,
                "Error generating binary filename for patient: {PatientId}, document: {DocumentId} [CorrelationId: {CorrelationId}]",
        ...
        string contentType)
    {
        ...
        try
        {
            ...
            _logger.LogDebug(
                "Generated embedded filename: {FileName} for patient: {PatientId}, document: {DocumentId}, index: {Index} [CorrelationId: {CorrelationId}]",
        ...
        {
            _logger.LogError(ex,
                "Error generating embedded filename for patient: {PatientId}, document: {DocumentId} [CorrelationId: {CorrelationId}]",
                patientId, documentRefId, correlationId);
            throw;
        }
    }

    public string GenerateS3Key(PracticeInfo practiceInfo,
        string patientId,
        string fileName,
        string? s3KeyPrefix = null)
    {
        ...
        try
        {
            string folderName = practiceInfo.GetFolderName();
            string sanitizedPatientId = SanitizeFileName(patientId);

            // Build S3 key with optional prefix: {prefix}/{practice}/{patientId}/{fileName}
            string s3Key = string.IsNullOrEmpty(s3KeyPrefix)
                ? $"{folderName}/{sanitizedPatientId}/{fileName}"
                : $"{s3KeyPrefix.TrimEnd('/')}/{folderName}/{sanitizedPatientId}/{fileName}";

            _logger.LogDebug(
                "Generated S3 key: {S3Key} for practice: {PracticeName}_{PracticeId}, patient: {PatientId}, file: {FileName}, prefix: {S3KeyPrefix} [CorrelationId: {CorrelationId}]",
                s3Key, practiceInfo.Name, practiceInfo.Id, patientId, fileName,
                s3KeyPrefix ?? "none", correlationId);
        ...
        {
            _logger.LogError(ex,
                "Error generating S3 key for practice: {PracticeName}_{PracticeId}, patient: {PatientId}, file: {FileName} [CorrelationId: {CorrelationId}]",
    ...
    public string GetFileExtensionFromContentType(string contentType)
    {
        ...
        string cleanContentType = contentType.Split(';')[0].Trim().ToLowerInvariant();

        if (ContentTypeExtensions.TryGetValue(cleanContentType, out string? extension))
            return extension;

        // Try to derive extension from content type
        if (cleanContentType.StartsWith("image/"))
        {
            string subtype = cleanContentType.Substring(6);
            return $".{subtype}";
        }

        if (cleanContentType.StartsWith("text/"))
        {
            string subtype = cleanContentType.Substring(5);
        ...
        _logger.LogWarning("Unknown content type: {ContentType}, using .bin extension",
            contentType);
        return ".bin";
    }
    ...
    public string SanitizeFileName(string fileName)
    {
        ...
        string sanitized = InvalidFileNameChars.Replace(fileName, "_");
    ...
    public bool ValidateFileNameComponents(string patientId,
        string documentRefId,
        string contentType)
    {
        if (string.IsNullOrWhiteSpace(patientId))
        ...
        {
            _logger.LogWarning("Document reference ID contains invalid characters: {DocumentRefId}",
                documentRefId);
            return false;
        }
