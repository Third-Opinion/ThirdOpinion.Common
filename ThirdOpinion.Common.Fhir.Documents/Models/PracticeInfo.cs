...
public class S3TagSet
{
    ...
    {
        if (!string.IsNullOrWhiteSpace(key) && !string.IsNullOrWhiteSpace(value)) Tags[key] = value;
    }

    public void AddCategoryTag(List<CodeableConcept> categories)
    {
        Coding? firstCategory = categories
    ...
    {
        EncounterReference? firstEncounter = encounters.FirstOrDefault();
    ...
    {
        if (meta?.LastUpdated != null) AddTag("meta.lastUpdated", meta.LastUpdated);

        if (meta?.VersionId != null) AddTag("meta.versionId", meta.VersionId);
    }
