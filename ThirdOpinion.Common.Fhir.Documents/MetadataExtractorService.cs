...
public class MetadataExtractorService : IMetadataExtractorService
{
    ...
    private static readonly Regex S3TagValueRegex
        = new(@"[^a-zA-Z0-9\+\-=\._:\/@\s]", RegexOptions.Compiled);
        ...
        Attachment? currentAttachment)
    {
        ...
        _logger.LogDebug(
            "Extracting metadata from DocumentReference {DocumentId} [CorrelationId: {CorrelationId}]",
        ...
        try
        {
            ...
            string? documentType = ExtractDocumentType(documentReference);
            if (!string.IsNullOrEmpty(documentType))
                tagSet.AddTag("document.type", SanitizeTagValue(documentType, 256));

            // Extract encounter reference
            string? encounterRef = ExtractEncounterReference(documentReference);
            ...
            {
                string lastUpdated = documentReference.Meta.LastUpdated ?? "";
                string versionId = documentReference.Meta.VersionId ?? "";
                if (!string.IsNullOrEmpty(lastUpdated) || !string.IsNullOrEmpty(versionId))
                {
                    ...
                    string formattedVersionId = !string.IsNullOrEmpty(versionId) &&
            ...
            if (!string.IsNullOrEmpty(documentReference.Date))
            {
                ...
                _logger.LogDebug(
                    "Added documentreference.date tag with value: {Date} [CorrelationId: {CorrelationId}]",
                    documentReference.Date, correlationId);
            }

            // Add DocumentReference ID with prefix for tracking
            tagSet.AddTag("documentreference.id",
                SanitizeTagValue($"DocumentReference/{documentReference.Id}"));

            // Extract patient ID if available
            string? patientId = documentReference.GetPatientId();
            ...
            if (practiceInfo != null && !string.IsNullOrEmpty(practiceInfo.Id))
            {
                ...
                string practiceNameToUse = resolvedPracticeName ?? practiceInfo.Name;

                // Combine practice.id and practice.name into single field
                string practiceValue = !string.IsNullOrEmpty(practiceNameToUse)
                ...
                _logger.LogDebug(
                    "Added practice.id tag with value: {PracticeValue} (resolved name: {WasResolved}) [CorrelationId: {CorrelationId}]",
            ...
            if (currentAttachment != null && !currentAttachment.IsEmbeddedContent)
                try
                {
                    string? binaryId = currentAttachment.GetBinaryId();
                    if (!string.IsNullOrEmpty(binaryId))
                    {
                        // If the binaryId already starts with "Binary/", use as-is, otherwise prepend
                        string binaryIdValue = binaryId.StartsWith("Binary/")
                            ? binaryId
                            : $"Binary/{binaryId}";
                        tagSet.AddTag("binary.id", SanitizeTagValue(binaryIdValue));
                        _logger.LogDebug(
                            "Added binary.id tag with value: {BinaryIdValue} [CorrelationId: {CorrelationId}]",
                            binaryIdValue, correlationId);
                    }
                    else
                    {
                        _logger.LogDebug(
                            "BinaryId was null or empty from GetBinaryId(). Attachment URL: {AttachmentUrl} [CorrelationId: {CorrelationId}]",
                ...
                catch (Exception ex)
                {
                    _logger.LogWarning(ex,
                        "Failed to extract binary ID from attachment. URL: {AttachmentUrl} [CorrelationId: {CorrelationId}]",
                        currentAttachment?.Url ?? "null", correlationId);
                }
            else
                _logger.LogDebug(
                    "currentAttachment is null or IsEmbeddedContent. currentAttachment != null: {IsNotNull}, IsEmbeddedContent: {IsEmbedded} [CorrelationId: {CorrelationId}]",
                    currentAttachment != null, currentAttachment?.IsEmbeddedContent ?? false,
                    correlationId);

            _logger.LogDebug(
                "Extracted {TagCount} metadata tags from DocumentReference {DocumentId} [CorrelationId: {CorrelationId}]",
        ...
        {
            _logger.LogError(ex,
                "Error extracting metadata from DocumentReference {DocumentId}. Error Type: {ErrorType}, Message: {ErrorMessage}, StackTrace: {StackTrace} [CorrelationId: {CorrelationId}]",
            ...
            if (ex.InnerException != null)
                _logger.LogError(ex.InnerException,
                    "Inner Exception - Type: {InnerErrorType}, Message: {InnerErrorMessage} [CorrelationId: {CorrelationId}]",
    ...
    public string? ExtractDocumentCategory(DocumentReferenceData documentReference)
    {
        ...
        {
            _logger.LogDebug(
                "No category found in DocumentReference {DocumentId} [CorrelationId: {CorrelationId}]",
                documentReference.Id, correlationId);
            return null;
        }
        ...
        foreach (CodeableConcept category in documentReference.Category)
        {
        ...
        {
            _logger.LogDebug(
                "No category codes found in DocumentReference {DocumentId} [CorrelationId: {CorrelationId}]",
                documentReference.Id, correlationId);
            return null;
        }
        ...
        if (categories.Count > 1)
            _logger.LogWarning(
                "Multiple categories found in DocumentReference {DocumentId}: {Categories}. Using first one. [CorrelationId: {CorrelationId}]",
    ...
    public string? ExtractDocumentType(DocumentReferenceData documentReference)
    {
        ...
        try
        {
            ...
            {
                _logger.LogDebug(
                    "No type found in DocumentReference {DocumentId} [CorrelationId: {CorrelationId}]",
                    documentReference.Id, correlationId);
                return null;
            }
            ...
            foreach (Coding coding in documentReference.Type.Coding)
            {
            ...
            {
                _logger.LogDebug(
                    "No type codes found in DocumentReference {DocumentId} [CorrelationId: {CorrelationId}]",
                    documentReference.Id, correlationId);
                return null;
            }
            ...
            _logger.LogDebug(
                "Extracted document type '{Type}' from DocumentReference {DocumentId} [CorrelationId: {CorrelationId}]",
        ...
        {
            _logger.LogError(ex,
                "Error extracting document type from DocumentReference {DocumentId} [CorrelationId: {CorrelationId}]",
                documentReference.Id, correlationId);
            return null;
        }
    }
    ...
    public string? ExtractEncounterReference(DocumentReferenceData documentReference)
    {
        ...
        if (documentReference.Context?.Encounter == null ||
            documentReference.Context.Encounter.Count == 0)
        {
            _logger.LogDebug(
                "No encounter context found in DocumentReference {DocumentId} [CorrelationId: {CorrelationId}]",
                documentReference.Id, correlationId);
            return null;
        }

        List<string> encounters = documentReference.Context.Encounter
        ...
        {
            _logger.LogDebug(
                "No encounter references found in DocumentReference {DocumentId} [CorrelationId: {CorrelationId}]",
                documentReference.Id, correlationId);
            return null;
        }
        ...
        if (encounters.Count > 1)
            _logger.LogWarning(
                "Multiple encounter references found in DocumentReference {DocumentId}: {Encounters}. Using first one. [CorrelationId: {CorrelationId}]",
    ...
    {
        if (string.IsNullOrEmpty(tagValue)) return "unknown";

        // Replace invalid characters with underscores
        string sanitized = S3TagValueRegex.Replace(tagValue, "_");
