...
public static class TestDataGenerators
{
    ...
    {
        public static (string email, string password, Dictionary<string, string> attributes) CreateTestUser(string? emailPrefix = null)
        {
            var firstName = _faker.Name.FirstName();
            var lastName = _faker.Name.LastName();
            var email = emailPrefix != null 
        ...
        public static List<(string email, string password, Dictionary<string, string> attributes)> CreateTestUsers(int count, string? emailPrefix = null)
        {
        ...
        private static string GenerateValidPassword()
        {
            ...
            password.Append(_faker.Random.String2(2, "abcdefghijklmnopqrstuvwxyz")); // Lowercase
            ...
            {
                password.Append(_faker.Random.String2(1, "ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789!@#$%^&*"));
            }
    ...
    public static class DynamoDb
    {
        ...
        public static Dictionary<string, object> CreateUserProfile(string userId)
        {
            return new Dictionary<string, object>
            {
                ...
                ["Roles"] = _faker.Make(_faker.Random.Int(1, 3), () => _faker.PickRandom("user", "admin", "moderator", "viewer"))
            };
        }
    }
    ...
    public static class S3
    {
        ...
        public static byte[] CreateImageData(int width = 100, int height = 100)
        {
            ...
            var headerSize = 54;
            var pixelDataSize = width * height * 3; // 24-bit RGB
        ...
        public static string CreateCsvContent(int rows = 10)
        {
            ...
            {
                csv.AppendLine($"{i + 1},{_faker.Name.FullName()},{_faker.Internet.Email()},{_faker.Random.Int(22, 65)},{_faker.Commerce.Department()},{_faker.Random.Decimal(35000, 120000)},{_faker.Date.Past().ToString("yyyy-MM-dd")}");
            }
        ...
        {
            var objectToSerialize = data ?? new
            {
                Id = Guid.NewGuid(),
                Name = _faker.Name.FullName(),
                Email = _faker.Internet.Email(),
            ...
            return System.Text.Json.JsonSerializer.Serialize(objectToSerialize, new System.Text.Json.JsonSerializerOptions 
            { 
                WriteIndented = true 
            });
        }
        ...
        public static Dictionary<string, string> CreateMetadata()
        {
            ...
            {
                ["content-type"] = _faker.PickRandom("application/json", "text/plain", "image/jpeg", "application/pdf"),
    ...
    public static class SQS
    {
        ...
        {
            var type = messageType ?? _faker.PickRandom("notification", "processing", "webhook", "alert");
            ...
            var message = new
            {
                ...
                Data = (object)(type switch
                {
                    ...
                    {
                        JobId = Guid.NewGuid(),
                        TaskType = _faker.PickRandom("image-resize", "document-convert", "data-export"),
                        InputPath = $"input/{_faker.System.FileName()}",
                        OutputPath = $"output/{_faker.System.FileName()}",
                        Parameters = _faker.Make<object>(3, () => new { Key = _faker.Lorem.Word(), Value = _faker.Lorem.Word() })
                    },
                    "webhook" => new
                    {
                        EventType = _faker.PickRandom("user.created", "order.completed", "payment.processed"),
        ...
        public static Dictionary<string, Amazon.SQS.Model.MessageAttributeValue> CreateMessageAttributes()
        {
            return new Dictionary<string, Amazon.SQS.Model.MessageAttributeValue>
            {
                ["MessageType"] = new() { DataType = "String", StringValue = _faker.PickRandom("notification", "processing", "webhook", "alert") },
                ["Priority"] = new() { DataType = "String", StringValue = _faker.PickRandom("low", "medium", "high") },
                ["Source"] = new() { DataType = "String", StringValue = _faker.Internet.DomainName() },
                ["UserId"] = new() { DataType = "String", StringValue = Guid.NewGuid().ToString() },
                ["Timestamp"] = new() { DataType = "Number", StringValue = DateTimeOffset.UtcNow.ToUnixTimeSeconds().ToString() },
                ["RetryCount"] = new() { DataType = "Number", StringValue = _faker.Random.Int(0, 3).ToString() }
            };
        }
    ...
    public static class CrossService
    {
        ...
        {
            public static (string userId, Dictionary<string, object> profile, byte[] profileImage, string welcomeMessage) CreateUserRegistrationData()
            {
                var userId = Guid.NewGuid().ToString();
                var profile = DynamoDb.CreateUserProfile(userId);
                var profileImage = S3.CreateImageData(200, 200);
                
                var welcomeMessage = System.Text.Json.JsonSerializer.Serialize(new
                {
                    UserId = userId,
                    ...
                    Timestamp = DateTime.UtcNow,
                    WelcomeMessage = $"Welcome {profile["FirstName"]}! Your account has been created successfully."
                });
        ...
        {
            public static (string documentId, string content, Dictionary<string, object> jobRecord, string processingMessage) CreateDocumentProcessingData(string userId)
            {
                var documentId = Guid.NewGuid().ToString();
                var content = S3.CreateTextContent(500);
                ...
                var processingMessage = System.Text.Json.JsonSerializer.Serialize(new
                {
                    JobId = jobRecord["Id"],
                    DocumentId = documentId,
                    UserId = userId,
        ...
        {
            public static (Dictionary<string, object> originalData, string backupData, string notificationMessage) CreateBackupData()
            {
                var originalData = DynamoDb.CreateTestRecord();
                var backupData = System.Text.Json.JsonSerializer.Serialize(originalData, new System.Text.Json.JsonSerializerOptions { WriteIndented = true });
    ...
    public static string GenerateUniqueName(string prefix = "test")
    {
        ...
        var randomSuffix = _faker.Random.AlphaNumeric(8).ToLowerInvariant();
