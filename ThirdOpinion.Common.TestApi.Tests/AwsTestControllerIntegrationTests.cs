...
public class AwsTestControllerIntegrationTests : IClassFixture<WebApplicationFactory<Program>>
{
    ...
    {
        // Arrange & Act
        HttpResponseMessage response = await _client.GetAsync("/api/aws-test/s3/test");
    ...
    {
        // Arrange & Act
        HttpResponseMessage response = await _client.GetAsync("/api/aws-test/dynamodb/test");
    ...
    {
        // Arrange & Act
        HttpResponseMessage response = await _client.GetAsync("/api/aws-test/sqs/test");
    ...
    {
        // Arrange & Act
        HttpResponseMessage response = await _client.GetAsync("/api/aws-test/cognito/test");
    ...
    {
        // Arrange & Act
        HttpResponseMessage response = await _client.GetAsync("/api/aws-test/test-all");

        // Assert
        response.StatusCode.ShouldBe(HttpStatusCode.OK);

        string content = await response.Content.ReadAsStringAsync();
        var results = JsonSerializer.Deserialize<Dictionary<string, TestSuiteResult>>(content,
        ...
        foreach (KeyValuePair<string, TestSuiteResult> kvp in results)
        {
