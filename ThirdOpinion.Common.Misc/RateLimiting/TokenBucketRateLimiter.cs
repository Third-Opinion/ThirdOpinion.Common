...
public class TokenBucketRateLimiter : IRateLimiter, IDisposable
{
        ...
        ServiceMetrics? metrics = null)
    {
        ...
        int burstSize = config.GetBurstSize();
        _tokens = burstSize;
        ...
        int refillIntervalMs = Math.Min(1000, Math.Max(50, (int)(1000 / config.CallsPerSecond)));
    ...
    {
        if (!_config.Enabled) return; // Rate limiting disabled, allow all requests
        ...
        try
        {
            ...
            lock (_lock)
            {
                ...
                {
                    _tokens--;
                    long waitTimeMs = stopwatch.ElapsedMilliseconds;
    ...
    private void RefillTokens(object? state)
    {
        ...
        lock (_lock)
        {
            ...
            if (elapsed <= 0) return; // Protect against clock adjustments

            // Calculate tokens to add based on elapsed time
            double tokensToAdd = elapsed * _config.CallsPerSecond;
            int maxTokens = _config.GetBurstSize();

            double previousTokens = _tokens;
            _tokens = Math.Min(_tokens + tokensToAdd, maxTokens);
            _lastRefillTime = now;

            // Release semaphore permits for the new tokens
            int newTokens = (int)_tokens - (int)previousTokens;
            if (newTokens > 0)
                try
                {
