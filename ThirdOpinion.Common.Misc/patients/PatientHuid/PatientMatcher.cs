...
public static class PatientMatcher
{
    ...
    public static float CalculateSamePerson(Patient patientBase, Patient patientToCompare)
    {
        ...
        {
            float lastNameSimilarity = CalculateStringSimilarity(
                patientBase.Demographics!.LastName!,
            ...
            !string.IsNullOrEmpty(patientToCompare.Demographics?.FirstName))
        {
            ...
            if (firstNameSimilarity < 0.7f)
            {
                ...
                string compareFirstName
                    = patientToCompare.Demographics!.FirstName!.Trim().ToLower();
            ...
            if ((patientBase.Demographics!.FirstName!.Length == 1 ||
                ...
                patientBase.Demographics!.FirstName![0] ==
                patientToCompare.Demographics!.FirstName![0])
            ...
            !string.IsNullOrEmpty(patientToCompare.Demographics?.MiddleName))
        {
            ...
            else
            {
                ...
                middleNameBonus
                    = middleNameSimilarity * 0.2f; // Up to 20% bonus for full middle name match
            }
        }
        ...
        // 3. SEX COMPARISON
        var sexScore = 0.0f;
        if (patientBase.Demographics?.Sex.HasValue == true &&
            ...
            patientToCompare.Demographics?.Sex.HasValue == true)
        {
            ...
            if (birthDateScore < 0.3f)
                samePerson
                    = Math.Min(samePerson, 0.1f); // Very low score for sex + birth date mismatch
            else
        ...
        if (patientBase.Demographics?.DeathDate.HasValue == true &&
            patientToCompare.Demographics?.DeathDate.HasValue == true &&
            patientBase.Demographics!.DeathDate!.Value.Date !=
            patientToCompare.Demographics!.DeathDate!.Value.Date)
