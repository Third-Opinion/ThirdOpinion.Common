...
public class DynamoDbRepository : IDynamoDbRepository
{
        ...
        CancellationToken cancellationToken = default)
    {
        try
        {
            ...
            _logger.LogDebug("Saved entity of type {EntityType} to DynamoDB table {TableName}",
                typeof(T).Name, config.OverrideTableName ?? "default");
        }
        catch (Exception ex)
        {
            _logger.LogError(ex,
                "Error saving entity of type {EntityType} to DynamoDB table {TableName}",
                typeof(T).Name, config.OverrideTableName ?? "default");
            throw;
        }
    }
        ...
        CancellationToken cancellationToken = default)
    {
        try
        {
            IBatchWrite<T>? batch = _context.CreateBatchWrite<T>();
        ...
        where T : class
    {
        try
        {
            ...
            if (rangeKey == null)
                return await _context.LoadAsync<T>(hashKey, loadConfig, cancellationToken);
        ...
        CancellationToken cancellationToken = default)
    {
        try
        {
            ...
            _logger.LogDebug(
                "Deleted entity of type {EntityType} with key {HashKey}/{RangeKey} from table {TableName}",
                typeof(T).Name, hashKey, rangeKey, config.OverrideTableName ?? "default");
        }
        catch (Exception ex)
        {
        ...
        CancellationToken cancellationToken = default)
    {
        try
        {
            ...
            if (filter != null)
            {
                ...
                IAsyncSearch<T>? q
                    = _context.QueryAsync<T>(hashKey, QueryOperator.BeginsWith, conditions);
                query = (AsyncSearch<T>)q;
            }
            else
            {
                IAsyncSearch<T>? q = _context.QueryAsync<T>(hashKey);
                query = (AsyncSearch<T>)q;
            }
        ...
        CancellationToken cancellationToken = default)
    {
        try
        {
            ...
            if (filter != null)
            {
                ...
                foreach (ScanCondition condition in filter.ToConditions())
                    conditions.Add(condition);
                IAsyncSearch<T>? s = _context.ScanAsync<T>(conditions);
                scan = (AsyncSearch<T>)s;
            }
            else
            {
                IAsyncSearch<T>? s = _context.ScanAsync<T>(new List<ScanCondition>());
                scan = (AsyncSearch<T>)s;
            }
