...
public class HealthLakeReaderService
{
    ...
    public async Task<DocumentContent?> GetDocumentReferenceContentAsync(string documentReferenceId,
        CancellationToken cancellationToken = default)
    {
        _logger.LogInformation("Retrieving DocumentReference {DocumentReferenceId} from HealthLake",
            documentReferenceId);
        ...
        try
        {
            ...
            var url
                = $"https://{endpoint}/datastore/{_config.DatastoreId}/r4/DocumentReference/{documentReferenceId}";
            ...
            if (!response.IsSuccessStatusCode)
            {
                ...
                _logger.LogError(
                    "Failed to retrieve DocumentReference {DocumentReferenceId}. Status: {StatusCode}, Content: {ErrorContent}",
            ...
            _logger.LogDebug("Received DocumentReference JSON: {JsonLength} characters",
                jsonContent.Length);
        ...
        {
            _logger.LogError(ex, "Error retrieving DocumentReference {DocumentReferenceId}",
                documentReferenceId);
            throw;
        }
    }
    ...
    /// </summary>
    private DocumentContent? ExtractDocumentContent(JsonElement documentReference,
        string documentReferenceId)
    {
        _logger.LogDebug("Extracting document content from DocumentReference {DocumentReferenceId}",
            documentReferenceId);

        try
        {
            // Navigate to content array
            if (!documentReference.TryGetProperty("content", out JsonElement contentArray) ||
                contentArray.ValueKind != JsonValueKind.Array)
            {
                _logger.LogWarning("DocumentReference {DocumentReferenceId} has no content array",
                    documentReferenceId);
                return null;
            }
            ...
            {
                _logger.LogWarning(
                    "DocumentReference {DocumentReferenceId} has empty content array",
                    documentReferenceId);
                return null;
            }
            ...
            if (!firstContent.TryGetProperty("attachment", out JsonElement attachment))
            {
                _logger.LogWarning(
                    "DocumentReference {DocumentReferenceId} content has no attachment",
                    documentReferenceId);
                return null;
            }

            // Extract base64 data
            if (!attachment.TryGetProperty("data", out JsonElement dataElement))
            {
                _logger.LogWarning(
                    "DocumentReference {DocumentReferenceId} attachment has no data field",
                    documentReferenceId);
                return null;
            }
            ...
            {
                _logger.LogWarning("DocumentReference {DocumentReferenceId} has empty base64 data",
                    documentReferenceId);
                return null;
            }
            ...
            if (attachment.TryGetProperty("contentType", out JsonElement contentTypeElement))
                mimeType = contentTypeElement.GetString();

            // Extract filename (optional)
            string? filename = null;
            if (attachment.TryGetProperty("title", out JsonElement titleElement))
                filename = titleElement.GetString();

            _logger.LogInformation(
                "Successfully extracted document content from DocumentReference {DocumentReferenceId}. " +
                "MIME Type: {MimeType}, Filename: {Filename}, Base64 Length: {Base64Length}",
                documentReferenceId, mimeType ?? "unknown", filename ?? "unknown",
                base64Data.Length);
            ...
            {
                _logger.LogError(ex,
                    "Failed to decode base64 data from DocumentReference {DocumentReferenceId}",
                    documentReferenceId);
                return null;
            }
        ...
        {
            _logger.LogError(ex,
                "Error extracting content from DocumentReference {DocumentReferenceId}",
                documentReferenceId);
            return null;
        }
    }
