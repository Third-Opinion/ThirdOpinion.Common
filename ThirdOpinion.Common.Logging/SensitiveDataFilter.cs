...
public class SensitiveDataLogger : ILogger
{
    ...
    {
        new(@"(?i)(access_token|bearer)\s*[=:]\s*[""']?([a-zA-Z0-9_\-\.]+)[""']?",
            RegexOptions.Compiled),
        new(@"(?i)(client_secret|secret)\s*[=:]\s*[""']?([a-zA-Z0-9_\-\.]+)[""']?",
            RegexOptions.Compiled),
        ...
        new(@"\b[A-Za-z0-9._%+-]+@[A-Za-z0-9.-]+\.[A-Z|a-z]{2,}\b",
            RegexOptions.Compiled) // Email pattern
    };
        ...
        foreach (Regex pattern in SensitivePatterns)
            sanitized = pattern.Replace(sanitized, match =>
            {
                ...
                if (groups.Count >= 3)
                {
                    ...
                    string sensitiveValue = groups[2].Value;
    ...
    private static string MaskSensitiveValue(string value)
    {
        ...
        return value.Substring(0, 2) + new string('*', value.Length - 4) +
...
public class ConsoleLogger : ILogger
{
        ...
        Func<TState, Exception?, string> formatter)
    {
        string message = formatter(state, exception);
        Console.WriteLine(
            $"[{DateTime.Now:yyyy-MM-dd HH:mm:ss.fff}] [{logLevel}] {_categoryName}: {message}");
