    ...
    public void Enrich(LogEvent logEvent, ILogEventPropertyFactory propertyFactory)
    {
        if (!string.IsNullOrEmpty(_instanceId))
            logEvent.AddPropertyIfAbsent(
                propertyFactory.CreateProperty("EC2InstanceId", _instanceId));

        if (!string.IsNullOrEmpty(_instanceType))
            logEvent.AddPropertyIfAbsent(
                propertyFactory.CreateProperty("EC2InstanceType", _instanceType));

        if (!string.IsNullOrEmpty(_availabilityZone))
            logEvent.AddPropertyIfAbsent(
                propertyFactory.CreateProperty("AvailabilityZone", _availabilityZone));
        ...
        if (!string.IsNullOrEmpty(_lambdaFunctionName))
            logEvent.AddPropertyIfAbsent(
                propertyFactory.CreateProperty("LambdaFunctionName", _lambdaFunctionName));

        if (!string.IsNullOrEmpty(_lambdaFunctionVersion))
            logEvent.AddPropertyIfAbsent(propertyFactory.CreateProperty("LambdaFunctionVersion",
                _lambdaFunctionVersion));

        // Add execution environment
        string executionEnvironment = DetermineExecutionEnvironment();
        logEvent.AddPropertyIfAbsent(
            propertyFactory.CreateProperty("ExecutionEnvironment", executionEnvironment));
    }
