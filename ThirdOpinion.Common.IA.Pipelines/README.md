...
using ThirdOpinion.DataFlow.Models;
...
public class MyService
{
    ...
    public async Task ProcessDataAsync()
    {
        ...
        await DataFlowPipeline<PatientData>
            ...
            .WithSource(PipelineSource<PatientData>.FromAsyncEnumerable(dataStream))
...
var context = _contextFactory
    ...
    .WithRunType(PipelineRunType.Fresh)      // Optional (defaults to Fresh)
    .WithParentRunId(null)                   // Optional parent run (for retries/continuations)
...
`PipelineSource<T>` encapsulates how input data is supplied for a run. Sources are resolved lazily, which allows you to branch based on `context.RunType`, load only incomplete resources during retries, or stream from external systems.

```csharp
var source = PipelineSource<MyResource>
    .FromRunType(
        progressService,
        freshSourceFactory: () => LoadFreshResources(),
        loadIncompleteAsync: (ids, ct) => FetchPendingResourcesAsync(ids, ct));

await DataFlowPipeline<MyResource>
    .Create(context, r => r.Id)
    .WithSource(source)
    .Transform(async r => await ProcessAsync(r), "Process")
    .Complete();
```

Available helpers:

- `FromEnumerable` / `FromAsyncEnumerable` (with optional context-aware overloads)
- `FromRunType` for retry/continuation scenarios
- `FromBlock` to plug in custom `ISourceBlock<T>`

You can also implement your own by instantiating `PipelineSource<T>` with a delegate that inspects `IPipelineContext`.
...
### With Amazon S3 Artifact Storage

```csharp
// Register an AWS S3 client (for example via the AWS SDK extensions)
...
services.AddThirdOpinionDataFlow()
    ...
    .UseDbContext<MyDataFlowDbContext>()
    .WithEntityFrameworkServices()
    .WithS3ArtifactStorage(options =>
    {
        options.BucketName = configuration["DataFlow:S3:BucketName"];
        options.KeyPrefix = "pipelines/artifacts";
    });
```
...
- `AWSSDK.S3` - Optional Amazon S3 artifact storage provider
