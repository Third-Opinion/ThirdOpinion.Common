...
2. Create Pipeline
   └─► DataFlowPipeline.Create<T>(context, idSelector)
       └─► WithSource(PipelineSource<T>.From…)

> **PipelineSource<T>**
>
> - Encapsulates how input data is materialized for a run.
> - Helpers include `FromEnumerable`, `FromAsyncEnumerable`, and `FromRunType` for retry/continuation scenarios.
> - Sources are resolved lazily when the first step is added, so you can branch based on `context.RunType` or other metadata.
...
### S3 Artifact Storage Provider

- Implementation: `Services\S3\S3ArtifactStorageService.cs`
- Configuration: `S3ArtifactStorageOptions` (bucket name, key prefix, optional bucket creation, concurrency)
- Dependencies: `IAmazonS3`, `PipelineContextPool`, `Microsoft.Extensions.Logging`
- Key layout: `runs/{PipelineRunId}/{ResourceId}/{StepName}/{ArtifactName}`
- Metadata persisted in `ArtifactEntity.MetadataJson` (size, content type, key, timestamps)
- Ensures bucket existence (optional) and stores references (`StoragePath = "s3://bucket/key"`)
- Registered via `.WithS3ArtifactStorage(...)` on the DataFlow builder
...
await DataFlowPipeline
    ...
    .WithSource(PipelineSource<Data>.FromEnumerable(testData))
