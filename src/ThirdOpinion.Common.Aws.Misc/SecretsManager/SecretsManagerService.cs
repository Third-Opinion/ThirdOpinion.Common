...
public class SecretsManagerService : ISecretsManagerService
{
    ...
    /// <inheritdoc />
    public async Task<Dictionary<string, string>> GetSecretAsync(CancellationToken cancellationToken = default)
    {
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        if (_config.EnableCaching && _cache.TryGetValue(cacheKey, out Dictionary<string, string>? cachedSecret))
        {
            _logger.LogDebug("Secret retrieved from cache: {SecretName}", secretName);
            return cachedSecret!;
        }

        _logger.LogDebug("Retrieving secret from AWS Secrets Manager: {SecretName} in region {Region}", 
            secretName, region);
        ...
        try
        {
            ...
            {
                throw new InvalidOperationException($"Secret '{secretName}' returned empty or null value");
            }

            // Parse the JSON secret
            var secretDictionary = ParseSecretJson(response.SecretString, secretName);
        ...
        {
            _logger.LogError(ex, "Secret not found: {SecretName} in region {Region}", secretName, region);
            throw new InvalidOperationException($"Secret '{secretName}' not found in region '{region}'", ex);
        }
        catch (AmazonSecretsManagerException ex) when (ex.ErrorCode == "AccessDeniedException")
        {
            _logger.LogError(ex, "Access denied to secret: {SecretName} in region {Region}", secretName, region);
            throw new UnauthorizedAccessException($"Access denied to secret '{secretName}' in region '{region}'", ex);
        }
        catch (AmazonSecretsManagerException ex) when (ex.ErrorCode == "InvalidParameterException")
        {
            ...
            throw new ArgumentException($"Invalid parameter for secret '{secretName}': {ex.Message}", ex);
        }
        catch (AmazonSecretsManagerException ex) when (ex.ErrorCode == "InvalidRequestException")
        {
            ...
            throw new InvalidOperationException($"Invalid request for secret '{secretName}': {ex.Message}", ex);
        }
        ...
        catch (AmazonSecretsManagerException ex) when (ex.ErrorCode == "InternalServiceErrorException")
        {
            _logger.LogError(ex, "AWS Secrets Manager internal error for secret: {SecretName}", secretName);
            throw new InvalidOperationException($"AWS Secrets Manager internal error for secret '{secretName}'", ex);
        }
        catch (AmazonSecretsManagerException ex)
        {
            _logger.LogError(ex, "AWS Secrets Manager error for secret: {SecretName}, ErrorCode: {ErrorCode}", 
                secretName, ex.ErrorCode);
            throw new InvalidOperationException($"AWS Secrets Manager error for secret '{secretName}': {ex.Message}", ex);
        }
        catch (JsonException ex)
        {
            _logger.LogError(ex, "Failed to parse secret JSON: {SecretName}", secretName);
            throw new InvalidOperationException($"Failed to parse secret '{secretName}' as valid JSON", ex);
        }
        catch (Exception ex)
        {
            _logger.LogError(ex, "Unexpected error retrieving secret: {SecretName}", secretName);
            throw new InvalidOperationException($"Unexpected error retrieving secret '{secretName}': {ex.Message}", ex);
        }
    }
    ...
    private static Dictionary<string, string> ParseSecretJson(string secretJson, string secretName)
    {
        try
        {
            using var jsonDocument = JsonDocument.Parse(secretJson);
        ...
        {
            throw new JsonException($"Failed to parse secret '{secretName}' as valid JSON: {ex.Message}", ex);
        }
    }
    ...
    private static bool IsTransientError(Exception exception)
    {
        if (exception is AmazonServiceException awsException)
        {
            ...
            return awsException.ErrorCode switch
            {
                ...
                _ when awsException.StatusCode == System.Net.HttpStatusCode.InternalServerError => true,
                _ when awsException.StatusCode == System.Net.HttpStatusCode.BadGateway => true,
                _ when awsException.StatusCode == System.Net.HttpStatusCode.ServiceUnavailable => true,
                _ when awsException.StatusCode == System.Net.HttpStatusCode.GatewayTimeout => true,
                _ when awsException.StatusCode == System.Net.HttpStatusCode.TooManyRequests => true,
                _ => false
            };
        }
