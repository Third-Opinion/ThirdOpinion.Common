...
public class AwsSignatureService : IAwsSignatureService
{
        ...
        CancellationToken cancellationToken = default)
    {
        try
        {
            ...
            return await SignRequestWithBodyAsync(request, requestBody ?? string.Empty, service, region, cancellationToken);
        }
        catch (Exception ex)
        {
            _logger.LogError(ex, "Failed to sign AWS request for service {Service} in region {Region}", service, region);
            throw;
        }
    }
        ...
        CancellationToken cancellationToken = default)
    {
        try
        {
            ...
            var maskedAccessKey = immutableCredentials.AccessKey.Length > 8
                ...
                : "***";
            var hasToken = !string.IsNullOrEmpty(immutableCredentials.Token);
            ...
            _logger.LogInformation("AWS Credentials - AccessKeyId: {AccessKey}, HasSessionToken: {HasToken}, Profile: {Profile}, Expiration: {Expiration}",
                maskedAccessKey,
                hasToken,
            ...
            var signer = new AwsSignatureVersion4.AwsSignatureHandlerSettings(
                region,
                service,
            ...
            {
                _logger.LogInformation("Encoding $ to %24 for AWS signature calculation: {OriginalPath}", canonicalUri);
                canonicalUri = canonicalUri.Replace("$", "%24");
            }
            else if (canonicalUri.Contains("%24"))
            {
                // Already encoded, use as-is for signature
                _logger.LogInformation("Path already contains %24 (encoded $), using for signature: {Path}", canonicalUri);
            }
            ...
            _logger.LogInformation("Request URL components - CanonicalUri: {CanonicalUri}, QueryString: {QueryString}, FullUrl: {FullUrl}",
            ...
            if (!string.IsNullOrEmpty(requestBody))
            {
                ...
                {
                    httpRequestMessage.Content.Headers.ContentType = request.Content.Headers.ContentType;
                }
                else
                {
            ...
            var contentHash = AWS4Signer.ComputeHash(requestBody ?? string.Empty);
        ...
        {
            _logger.LogError(ex, "Failed to sign request with body for service {Service} in region {Region}", service, region);
            throw;
        }
    }
        ...
        string amzDate)
    {
        ...
        {
            headersToSign.Add(new KeyValuePair<string, string>("x-amz-security-token", headers["x-amz-security-token"]));
        }

        // Include x-amz-content-sha256 if present (only added for non-GET requests)
        if (headers.ContainsKey("x-amz-content-sha256"))
        {
            headersToSign.Add(new KeyValuePair<string, string>("x-amz-content-sha256", headers["x-amz-content-sha256"]));
        }

        var sortedHeaders = headersToSign.OrderBy(h => h.Key).ToList();
        var canonicalHeaders = string.Join("\n", sortedHeaders.Select(h => $"{h.Key}:{h.Value.Trim()}")) + "\n";
        var signedHeaders = string.Join(";", sortedHeaders.Select(h => h.Key));

        _logger.LogDebug("AWS Signature Headers - Canonical: {CanonicalHeaders}, Signed: {SignedHeaders}",
        ...
        _logger.LogDebug("Canonical Request Components - Method: {Method}, URI: {URI}, Query: {Query}, PayloadHash: {PayloadHash}",
            httpMethod, canonicalUri, queryString, payloadHash);

        // Create the string to sign
        var algorithm = "AWS4-HMAC-SHA256";
        var credentialScope = $"{dateStamp}/{region}/{service}/aws4_request";
        var hashedCanonicalRequest = AWS4Signer.ComputeHash(canonicalRequest);
        var stringToSign = $"{algorithm}\n{amzDate}\n{credentialScope}\n{hashedCanonicalRequest}";

        // Calculate the signature
        var signingKey = AWS4Signer.GetSignatureKey(credentials.SecretKey, dateStamp, region, service);
        var signature = AWS4Signer.HmacSha256(stringToSign, signingKey);

        // Create the authorization header
        var authorizationHeader = $"{algorithm} Credential={credentials.AccessKey}/{credentialScope}, SignedHeaders={signedHeaders}, Signature={signature}";

        _logger.LogInformation("Generated Authorization Header: {AuthorizationHeader}", authorizationHeader);
    ...
    private string EncodeQueryStringForAwsSignature(string queryString)
    {
        ...
        try
        {
            ...
            var encodedPairs = parameters.Select(p => $"{p.Key}={p.Value}");
            var result = string.Join("&", encodedPairs);

            _logger.LogDebug("Encoded query string for AWS signature: Original={Original}, Encoded={Encoded}",
                queryString, result);
        ...
        {
            _logger.LogWarning(ex, "Failed to encode query string for AWS signature, using original: {QueryString}", queryString);
            return queryString;
        }
    }
}