...
public class RetryPolicyService : IRetryPolicyService
{
    ...
    private readonly ConcurrentDictionary<string, IAsyncPolicy<HttpResponseMessage>> _combinedPolicies = new();
    ...
    private IAsyncPolicy<HttpResponseMessage> CreateCircuitBreakerPolicy(string serviceName)
    {
        return Policy
            ...
            .CircuitBreakerAsync(
                ...
                {
                    _logger.LogWarning("Circuit breaker opened for service {ServiceName}. Duration: {Duration}. Exception: {Exception}",
    ...
    private static bool ShouldRetryHttpException(HttpRequestException ex)
    {
        ...
        if (message.Contains("400") || message.Contains("badrequest") || message.Contains("bad request") ||
            message.Contains("401") || message.Contains("unauthorized") ||
            message.Contains("403") || message.Contains("forbidden") ||
            message.Contains("404") || message.Contains("notfound") || message.Contains("not found") ||
        ...
        return message.Contains("500") || message.Contains("internalservererror") || message.Contains("internal server error") ||
               message.Contains("502") || message.Contains("badgateway") || message.Contains("bad gateway") ||
               message.Contains("503") || message.Contains("serviceunavailable") || message.Contains("service unavailable") ||
               message.Contains("504") || message.Contains("gatewaytimeout") || message.Contains("gateway timeout") ||
               message.Contains("429") || message.Contains("toomanyrequests") || message.Contains("too many requests") ||
    ...
    private TimeSpan CalculateDelay(int retryAttempt)
    {
        ...
        var jitter = exponentialDelay * jitterPercentage * (_random.NextDouble() - 0.5);
        ...
        int retryCount)
    {
        ...
        {
            _logger.LogWarning("Retry attempt {RetryCount} for {ServiceName} after exception: {Exception}. " +
                             "Next attempt in {Delay}ms",
                retryCount, serviceName, outcome.Exception.Message, delay.TotalMilliseconds);
        }
        else if (outcome.Result != null)
        {
            _logger.LogWarning("Retry attempt {RetryCount} for {ServiceName} after HTTP {StatusCode}. " +
                             "Next attempt in {Delay}ms",
    ...
    {
        _logger.LogWarning("Manual circuit breaker reset requested for service: {ServiceName}", serviceName);
        ...
        _logger.LogInformation("Circuit breaker reset completed for service: {ServiceName}", serviceName);
    }
}