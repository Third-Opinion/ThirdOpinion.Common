...
public class HealthLakeDocumentDownloadService
{
    ...
    /// <param name="specificDocumentReferenceId">Optional specific DocumentReference ID to download (if provided, only this one will be downloaded)</param>
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        try
        {
            ...
            do
            {
                ...
                var documentReferences = bundle.GetDocumentReferences();
            ...
            foreach (var docRef in allDocumentReferences)
            {
            ...
            foreach (var docRef in allDocumentReferences)
            {
                ...
                foreach (var content in docRef.Content)
                {
                    ...
                    {
                        var contentType = content.Attachment.ContentType ?? "unknown";
                        contentTypes.Add(contentType);
                    }
                }

                var contentCount = docRef.Content?.Count ?? 0;
                var contentTypeStr
            ...
            for (int i = 0; i < allDocumentReferences.Count; i++)
            {
                var documentReference = allDocumentReferences[i];
                ...
                try
                {
                    ...
                    var documentResult = await ProcessDocumentReferenceDirectAsync(
                        documentReference,
                        patientId,
                        overridePracticeId,
                    ...
                    {
                        var currentProgress = (double)(i + 1) / allDocumentReferences.Count * 100;
                        _logger.LogInformation(
            ...
            var successfulDocuments = results.Count(r => r.Success);
            var failedDocuments = results.Count(r => !r.Success);
        ...
        catch (Exception ex)
        {
            _logger.LogError(ex,
                ...
                patientId, correlationId);
            throw new DocumentDownloadException(
                $"Failed to get DocumentReferences for Patient {patientId}: {ex.Message}", ex, true,
    ...
    {
        var correlationId = _correlationIdProvider.GetCorrelationId();
        var documentReferenceId = documentReference.Id!;
        ...
        try
        {
            ...
            if (documentReference.Status?.Equals("entered-in-error", StringComparison.OrdinalIgnoreCase) == true)
            {
                _logger.LogWarning(
            ...
            var practiceId = overridePracticeId ?? documentReference.GetPracticeInfo()?.Id;
            var practiceName = overridePracticeName ?? documentReference.GetPracticeInfo()?.Name;
            ...
            for (int attachmentIndex = 0;
                 ...
                 attachmentIndex++)
            {
                var content = documentReference.Content[attachmentIndex];
                ...
                try
                {
                    var fileResult = await ProcessAttachmentAsync(
                        documentReference,
                        attachment,
                        attachmentIndex,
                        patientId,
        ...
        try
        {
            ...
            else
            {
                ...
                try
                {
                    ...
                    var requestUrl = $"https://healthlake.{_healthLakeConfig.Region}.amazonaws.com/datastore/{_healthLakeConfig.DatastoreId}/r4/DocumentReference/{documentReferenceId}";

                    using var request = new HttpRequestMessage(HttpMethod.Get, requestUrl);
                    request.Headers.Add("Accept", "application/fhir+json");

                    using var response = await _healthLakeHttpService.SendSignedRequestAsync(request, cancellationToken);
                    ...
                    if (!response.IsSuccessStatusCode)
                    {
                        ...
                        throw new DocumentDownloadException(
                            ...
                            true, ErrorCategory.Infrastructure);
                    }

                    var documentReferenceJson = await response.Content.ReadAsStringAsync(cancellationToken);

                    if (string.IsNullOrEmpty(documentReferenceJson))
                    {
                        throw new DocumentDownloadException(
                            $"Empty response from HealthLake for DocumentReference {documentReferenceId}",
                            false, ErrorCategory.Infrastructure);
                    }

                    // Parse the DocumentReference JSON into our data model
                    documentReference = JsonSerializer.Deserialize<DocumentReferenceData>(documentReferenceJson);
                ...
                catch (Exception ex) when (!(ex is DocumentDownloadException))
                {
                    throw new DocumentDownloadException(
                        ...
                        ex, true, ErrorCategory.Infrastructure);
                }
            }
            ...
            var patientId = overridePatientId ?? documentReference.GetPatientId();
            var practiceInfo = documentReference.GetPracticeInfo();
            var practiceId = overridePracticeId ?? practiceInfo?.Id;
        ...
        CancellationToken cancellationToken)
    {
        ...
        try
        {
            ...
            var decodedContent = await _base64ContentExtractor.ExtractAndDecodeAsync(attachment, cancellationToken);

            // Generate S3 key for embedded content pat/docref....`
            var fileName = _fileOrganizationService.GenerateEmbeddedFileName(
                patientId,
                documentReferenceId,
                attachmentIndex,
            ...
            var s3Key = _fileOrganizationService.GenerateS3Key(practiceInfo,patientId, fileName, s3KeyPrefix);

            // Use the actual S3 bucket provided or fall back to default
            var actualS3Bucket = s3Bucket ?? _s3BucketName;
            ...
            var putRequest = new PutObjectRequest
            {
                ...
                ContentType = decodedContent.ContentType ?? attachment.ContentType ?? "application/octet-stream",
            ...
            {
                putRequest.TagSet = s3TagSet.Tags.Select(tag => new Tag { Key = tag.Key, Value = tag.Value }).ToList();
            }

            // Set server-side encryption - TODO: Make configurable
            putRequest.ServerSideEncryptionMethod = ServerSideEncryptionMethod.AES256;

            var uploadResponse = await _s3Client.PutObjectAsync(putRequest, cancellationToken);
            ...
            _logger.LogInformation("Successfully uploaded embedded file to S3: " +
                ...
                s3Key, actualS3Bucket, documentReferenceId, attachmentIndex, (DateTime.UtcNow - startTime).TotalMilliseconds, correlationId);
        ...
        {
            _logger.LogError(ex, "Failed to process embedded attachment[{Index}] for DocumentReference {DocumentReferenceId} [CorrelationId: {CorrelationId}]",
                attachmentIndex, documentReferenceId, correlationId);
    ...
    private async Task<FileDownloadResult> ProcessBinaryAttachmentAsync(
        ...
        CancellationToken cancellationToken)
    {
        var binaryId = attachment.GetBinaryId();
        ...
        var fileName = _fileOrganizationService.GenerateBinaryFileName(
            patientId,
            documentReferenceId,
            attachmentIndex,
            attachment.Title,
        ...
        var downloadResult = await _binaryDownloadService.DownloadBinaryToS3Async(
            binaryId,
            s3Bucket ?? throw new ArgumentNullException(nameof(s3Bucket), "S3 bucket name is required for binary downloads"),
            s3Key,
            s3TagSet,
            patientId,
            documentReferenceId,
    ...
    private void LogDetailedErrorReport(List<DocumentDownloadResults> results, string patientId)
    {
        ...
        {
            _logger.LogInformation("All DocumentReferences processed successfully for patient {PatientId}", patientId);
            return;
        }
        ...
        {
            _logger.LogError("--- DocumentReference: {DocumentReferenceId} ---", result.DocumentReferenceId);
            ...
            {
                var failedFiles = result.FileResults.Where(f => !f.Success).ToList();
                if (failedFiles.Any())
                {
                    _logger.LogError("Failed files ({FailedFileCount}/{TotalFileCount}):",
                        failedFiles.Count, result.FileResults.Count);

                    foreach (var fileResult in failedFiles)
                    {
                        _logger.LogError("  - File: {FileName}, ContentType: {ContentType}, Error: {ErrorMessage}",
            ...
            else
            {
                ...
                {
                    _logger.LogError("Issue: DocumentReference has content attachments but none have data or URL");
                }
                else if (result.ErrorMessage?.Contains("not found") == true)
                {
                    _logger.LogError("Issue: DocumentReference or referenced Binary resource not found");
                }
                else if (result.ErrorMessage?.Contains("access denied") == true || result.ErrorMessage?.Contains("unauthorized") == true)
                {
                    _logger.LogError("Issue: Access denied when trying to retrieve DocumentReference or Binary");
                }
