...
public class BundleParserService : IBundleParserService
{
    ...
    public BundleData ParseBundle(string bundleJson)
    {
        ...
        try
        {
            _logger.LogDebug("Parsing Bundle JSON (length: {Length}) [CorrelationId: {CorrelationId}]",
            ...
            _logger.LogDebug("Successfully parsed Bundle: Type={Type}, Total={Total}, Entries={EntryCount} [CorrelationId: {CorrelationId}]",
            ...
            var documentReferences = ProcessDocumentReferenceEntries(bundle);

            _logger.LogInformation("Extracted {Count} DocumentReference resources from Bundle [CorrelationId: {CorrelationId}]",
        ...
        {
            _logger.LogError(ex, "Failed to parse Bundle JSON [CorrelationId: {CorrelationId}]", correlationId);
            throw new DocumentDownloadException("Invalid Bundle JSON format", ex);
        }
        catch (Exception ex)
        {
            _logger.LogError(ex, "Unexpected error parsing Bundle [CorrelationId: {CorrelationId}]", correlationId);
            throw;
        }
    }
    ...
    public PracticeInfo? ExtractPracticeInfo(DocumentReferenceData documentReference)
    {
        ...
        try
        {
            var practiceExtension = documentReference.Extension
            ...
            {
                _logger.LogWarning("No practice extension found in DocumentReference: {DocumentId} [CorrelationId: {CorrelationId}]",
                    documentReference.Id, correlationId);
                return null;
            }

            var practiceReference = practiceExtension.ValueReference.Reference;
            _logger.LogDebug("Found practice reference: {Reference} [CorrelationId: {CorrelationId}]",
            ...
            if (parts?.Length >= 2)
            {
                var practiceId = parts[1];

                _logger.LogDebug("Extracted practice ID: {PracticeId} from reference: {Reference} [CorrelationId: {CorrelationId}]",
            ...
            _logger.LogWarning("Failed to parse practice ID from reference: {Reference} [CorrelationId: {CorrelationId}]",
                practiceReference, correlationId);
            return null;
        }
        catch (Exception ex)
        {
            _logger.LogError(ex, "Error extracting practice info from DocumentReference: {DocumentId} [CorrelationId: {CorrelationId}]",
                documentReference.Id, correlationId);
            return null;
        }
    }

    public async Task<string> ResolvePracticeNameAsync(string practiceId, CancellationToken cancellationToken = default)
    {
        var correlationId = _correlationIdProvider.GetCorrelationId();

        _logger.LogDebug("Resolving practice name for ID: {PracticeId} [CorrelationId: {CorrelationId}]",
        ...
        var practiceName = practiceId;

        _logger.LogDebug("Resolved practice name: {PracticeName} for ID: {PracticeId} [CorrelationId: {CorrelationId}]",
    ...
    private List<DocumentReferenceData> ProcessDocumentReferenceEntries(BundleData bundle)
    {
        ...
        foreach (var entry in bundle.Entry)
        {
            try
            {
                ...
                {
                    _logger.LogWarning("Bundle entry has null resource [CorrelationId: {CorrelationId}]", correlationId);
                    continue;
                }
                ...
                {
                    _logger.LogWarning("DocumentReference has empty ID [CorrelationId: {CorrelationId}]", correlationId);
                    continue;
                }

                documentReferences.Add(entry.Resource);

                _logger.LogDebug("Processed DocumentReference: {DocumentId}, Status: {Status}, Content Count: {ContentCount} [CorrelationId: {CorrelationId}]",
                    entry.Resource.Id, entry.Resource.Status, entry.Resource.Content.Count, correlationId);
            }
            catch (Exception ex)
            {
                _logger.LogError(ex, "Error processing Bundle entry for DocumentReference [CorrelationId: {CorrelationId}]", correlationId);
