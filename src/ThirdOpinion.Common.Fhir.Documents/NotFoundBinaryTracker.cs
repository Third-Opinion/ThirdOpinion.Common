...
public class NotFoundBinaryTracker : INotFoundBinaryTracker
{
        ...
        ICorrelationIdProvider correlationIdProvider)
    {
        ...
        _notFoundLogFilePath = Path.Combine(Directory.GetCurrentDirectory(), $"not_found_binaries_{timestamp}.csv");
    ...
    public string GetNotFoundLogFilePath() => _notFoundLogFilePath;
    ...
    private void EnsureHeadersExist()
    {
        if (!File.Exists(_notFoundLogFilePath))
        {
            ...
            _logger.LogInformation("Created NotFound binaries log file: {LogFilePath}", _notFoundLogFilePath);
        }
    }

    private async Task WriteToCsvFileAsync(NotFoundBinaryRecord record, CancellationToken cancellationToken)
    {
        await _fileLock.WaitAsync(cancellationToken);
        try
        {
            var csvLine = $"{record.Timestamp:yyyy-MM-dd HH:mm:ss.fff}," +
            ...
            await File.AppendAllTextAsync(_notFoundLogFilePath, csvLine + Environment.NewLine, cancellationToken);

            _logger.LogDebug("Wrote NotFound binary record to CSV: {BinaryId}", record.BinaryId);
        }
        catch (Exception ex)
        {
            _logger.LogError(ex, "Failed to write NotFound binary record to CSV file: {LogFilePath}", _notFoundLogFilePath);
        }
        finally
        {
            _fileLock.Release();
