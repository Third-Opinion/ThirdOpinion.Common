...
public class FileNamingService : IFileNamingService
{
    ...
    /// <inheritdoc />
    public string GenerateOutputFileName(string originalName, string version, string runId, PromptType promptType)
    {
        if (string.IsNullOrWhiteSpace(originalName))
            throw new ArgumentException("Original name cannot be null or whitespace", nameof(originalName));
        ...
        var extension = GetExtensionForPromptType(promptType);
        ...
        if (fileName.Length > maxFileNameLength)
        {
            ...
            var availableLength = maxFileNameLength - $"_v{sanitizedVersion}_{sanitizedRunId}{extension}".Length;
            if (availableLength > 0)
            {
                sanitizedBaseName = sanitizedBaseName[..Math.Min(sanitizedBaseName.Length, availableLength)];
                fileName = $"{sanitizedBaseName}_v{sanitizedVersion}_{sanitizedRunId}{extension}";
            }
            else
            {
                throw new ArgumentException($"Version and run ID are too long to generate a valid filename. Combined length exceeds {maxFileNameLength} characters.");
            }
        }

        _logger.LogDebug("Generated filename: {FileName} from original: {OriginalName}, version: {Version}, runId: {RunId}, promptType: {PromptType}",
    ...
    /// <inheritdoc />
    public string GenerateOutputFileName(string originalName, int version, string runId, PromptType promptType)
    {
    ...
    public string GetExtensionForPromptType(PromptType promptType)
    {
        return promptType switch
        {
            ...
            _ => throw new ArgumentOutOfRangeException(nameof(promptType), promptType, "Unknown prompt type")
        };
    }
    ...
    public PromptType DeterminePromptTypeFromName(string promptName)
    {
        ...
        var lowerPromptName = promptName.ToLowerInvariant();
        ...
        return lowerPromptName switch
        {
            ...
            _ when lowerPromptName.Contains("markdown") || lowerPromptName.Contains("md") => PromptType.Markdown,
    ...
    public string SanitizeFileName(string fileName)
    {
        ...
        var sanitized = InvalidFileNameCharsRegex.Replace(fileName, "_");
    ...
    public bool IsValidFileName(string fileName)
    {
        ...
        var baseNameUpper = Path.GetFileNameWithoutExtension(fileName).ToUpperInvariant();
    ...
    public string GenerateVersionedFileName(string baseName, int version, string? suffix = null)
    {
        ...
        var suffixPart = !string.IsNullOrEmpty(suffix) ? $"_{SanitizeFileName(suffix)}" : "";
    ...
    public (string baseName, int version, string? suffix) ParseVersionedFileName(string fileName)
    {
        ...
        var versionMatch = Regex.Match(nameWithoutExtension, @"_v(\d+)(?:_(.+))?$");
        ...
        var suffix = versionMatch.Groups[2].Success ? versionMatch.Groups[2].Value : null;
        var baseName = nameWithoutExtension[..versionMatch.Index];
