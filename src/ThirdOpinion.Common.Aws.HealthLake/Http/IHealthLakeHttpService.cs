...
public interface IHealthLakeHttpService
{
    ...
    Task<HttpResponseMessage> SendSignedRequestAsync(HttpRequestMessage request, CancellationToken cancellationToken = default);
    ...
    /// <param name="cancellationToken">Cancellation token</param>
    /// <returns>The HTTP response</returns>
    Task<HttpResponseMessage> SendSignedRequestAsync(HttpRequestMessage request, string serviceName, CancellationToken cancellationToken = default);
