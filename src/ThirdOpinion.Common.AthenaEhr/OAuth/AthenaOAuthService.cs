...
public class AthenaOAuthService : IAthenaOAuthService
{
    ...
    public async Task<OAuthToken> GetTokenAsync(CancellationToken cancellationToken = default)
    {
        ...
        if (_config.UseTokenCache && _tokenCache.TryGetValue(cacheKey, out var cachedToken))
        {
            if (cachedToken != null && !cachedToken.IsExpired())
            {
                _logger.LogDebug("Returning cached OAuth token for practice {PracticeId}", _config.PracticeId);
                return cachedToken;
            }
        }

        _logger.LogInformation("Requesting new OAuth token for practice {PracticeId}", _config.PracticeId);
        ...
        try
        {
            ...
            if (response.IsSuccessStatusCode)
            {
                ...
                if (_config.UseTokenCache)
                {
                    ...
                    _logger.LogDebug("Cached OAuth token for practice {PracticeId}", _config.PracticeId);
                }

                _logger.LogInformation("Successfully obtained OAuth token for practice {PracticeId}", _config.PracticeId);
                return token;
            }
            ...
            throw new OAuthException($"Failed to obtain OAuth token: {response.StatusCode} - {errorContent}");
        }
    ...
    public Task<OAuthToken> RefreshTokenAsync(CancellationToken cancellationToken = default)
    {
        ...
        _logger.LogInformation("Athena OAuth uses client_credentials; requesting new token instead of refresh");
