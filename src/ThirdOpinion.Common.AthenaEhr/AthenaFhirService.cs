...
public class AthenaFhirService : IFhirSourceService
{
    ...
    public async Task<string?> GetResourceAsync(string resourceType, string resourceId, CancellationToken cancellationToken = default)
    {
        ...
        {
            // Get OAuth token
            var token = await _oauthService.GetTokenAsync(cancellationToken);
            ...
            if (response.IsSuccessStatusCode)
            {
                ...
                _logger.LogInformation("Successfully retrieved FHIR resource from Athena: {ResourceType}/{ResourceId}",
                    resourceType, resourceId);
                return content;
            }
        ...
        {
            _logger.LogError(ex, "Error retrieving FHIR resource from Athena: {ResourceType}/{ResourceId}",
                resourceType, resourceId);
    ...
    /// <inheritdoc />
    public async Task<string> SearchResourcesAsync(string resourceType, string searchParams, CancellationToken cancellationToken = default)
    {
        ValidateResourceType(resourceType);

        _logger.LogInformation("Starting Athena search operation for {ResourceType} with params: {SearchParams}",
        ...
        try
        {
            ...
            {
                var content = await response.Content.ReadAsStringAsync(cancellationToken);
                _logger.LogInformation("Successfully searched FHIR resources from Athena: {ResourceType}", resourceType);
                return content;
            }
        ...
        {
            _logger.LogError(ex, "Error searching FHIR resources from Athena: {ResourceType}", resourceType);
            throw new FhirResourceException(
    ...
    /// <inheritdoc />
    public async Task<T?> GetResourceAsync<T>(string resourceType, string resourceId, CancellationToken cancellationToken = default) where T : class
    {
        var json = await GetResourceAsync(resourceType, resourceId, cancellationToken);
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        _logger.LogInformation("Starting batch Athena GET operation for {Count} resources", resourceList.Count);
        ...
        var tasks = resourceList.Select(async resource =>
        {
            ...
            try
            {
                var content = await GetResourceAsync(resource.ResourceType, resource.ResourceId, cancellationToken);
                lock (results)
                {
                    results[key] = content;
                }
        ...
        _logger.LogInformation("Completed batch Athena GET operation: {SuccessCount}/{TotalCount} successful",
    ...
    private async Task HandleErrorResponseAsync(HttpResponseMessage response, string resourceType, string? resourceId, CancellationToken cancellationToken)
    {
        var content = await response.Content.ReadAsStringAsync(cancellationToken);
        ...
        switch (response.StatusCode)
        {
            ...
            case HttpStatusCode.Unauthorized:
                throw new FhirAuthenticationException("Authentication failed for Athena API access");
