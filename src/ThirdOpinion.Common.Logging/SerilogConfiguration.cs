...
public static class SerilogConfiguration
{
    ...
    public static Serilog.Core.Logger CreateLogger(IConfiguration configuration, string applicationName)
    {
        ...
        var outputTemplate = "[{Timestamp:yyyy-MM-dd HH:mm:ss.fff zzz}] [{Level:u3}] [{CorrelationId}] {Message:lj}{NewLine}{Exception}";
        ...
        var logPath = configuration["Logging:FilePath"] ?? "logs/application-.json";
    ...
    /// </summary>
    public static ILoggingBuilder AddSerilog(this ILoggingBuilder loggingBuilder, Serilog.Core.Logger logger)
    {
    ...
    /// </summary>
    public static IServiceCollection AddStructuredLogging(this IServiceCollection services, IConfiguration configuration, string applicationName)
    {
        var logger = CreateLogger(configuration, applicationName);
        Log.Logger = logger;
...
public class CorrelationIdEnricher : Serilog.Core.ILogEventEnricher
{
    public void Enrich(Serilog.Events.LogEvent logEvent, Serilog.Core.ILogEventPropertyFactory propertyFactory)
    {
        ...
        var correlationId = CorrelationIdHelper.GetCurrentCorrelationId();
