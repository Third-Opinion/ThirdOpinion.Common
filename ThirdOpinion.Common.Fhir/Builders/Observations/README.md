...
### RadiographicProgressionObservationBuilder

Builder for creating FHIR Observations for radiographic disease progression assessment when RECIST criteria are not applicable (e.g., insufficient baseline measurements, non-measurable disease).

#### When to Use

Use this builder when:
- Baseline target lesion measurements are not available for RECIST 1.1 scoring
- Assessing non-measurable disease progression
- Performing qualitative radiographic assessment without formal RECIST criteria
- New lesions are detected but formal RECIST measurements cannot be applied
- Disease burden has qualitatively increased but lacks quantitative baseline data

#### Key Features

- **SNOMED-based progression status** - Uses SNOMED codes for progression detected (162573006) or not detected (260415000)
- **Qualitative assessment** - Supports descriptive assessment when quantitative measurements unavailable
- **Flexible body site tracking** - Add multiple body sites as components
- **Clinical notes** - Attach detailed notes explaining the assessment rationale
- **Imaging references** - Link to imaging studies and radiology reports
- **AI confidence** - Include AI confidence scores and rationale
...
{
    ModelName = "radiographic-assessment-v1",
    ModelVersion = "1.0"
};

var progressionObservation = new RadiographicProgressionObservationBuilder(config)
    .WithInferenceId("radiographic-assessment-001")
    .WithPatient("Patient/patient-456", "Jane Smith")
    .WithDevice("Device/radiographic-ai-v2", "Radiographic AI Analyzer v2.0")
    .WithFocus("Condition/prostate-cancer")
    .WithProgressionDetected() // Or .WithNoProgressionDetected()
    ...
    .AddBodySite("10200004", "Liver structure") // New liver lesion
    .AddBodySite("181422007", "Entire prostate") // Primary site
    .WithImagingType("CT")
    .WithImagingDate(DateTime.Parse("2024-11-02"))
    .WithConfidence(0.85f)
    .AddNote("Assessment based on new lesions and qualitative increase in disease burden. Formal RECIST 1.1 scoring not possible due to lack of baseline target lesion measurements.")
    .Build();
```

#### Setting Progression Status

Three ways to set the progression status:

```csharp
// 1. Convenience method - progression detected
.WithProgressionDetected()

// 2. Convenience method - no progression
.WithNoProgressionDetected()

// 3. Custom SNOMED code
.WithProgressionStatus("162573006", "Progression of disease")
```

#### Adding Notes and Rationale

```csharp
.AddNote("New 2.5cm liver lesion identified on current CT scan")
.AddNote("Qualitative increase in disease burden compared to prior imaging")
.WithQualitativeAssessment("Multiple new hepatic lesions with increased tumor burden")
.WithConfidenceRationale("High confidence based on clear new lesion detection and comparison with prior imaging")
```
...
Add custom components for additional context:
...
// Quantity measurements (if available)
...
#### Example: Progression with New Lesions

```csharp
var progressionObservation = new RadiographicProgressionObservationBuilder(config)
    .WithInferenceId("rad-prog-001")
    .WithPatient("Patient/pt-123")
    .WithDevice("Device/ai-radiographic-v2")
    .WithFocus("Condition/metastatic-prostate-cancer")
    .WithProgressionDetected()
    .AddImagingStudy("ImagingStudy/ct-2024-11-02")
    .AddRadiologyReport("DocumentReference/rad-report-123")
    .AddBodySite("10200004", "Liver structure")
    .AddBodySite("39607008", "Lung structure")
    .WithImagingType("CT")
    .WithImagingDate(DateTime.UtcNow)
    .WithConfidence(0.92f)
    .WithQualitativeAssessment("Multiple new hepatic and pulmonary lesions consistent with metastatic progression")
    .AddNote("New hepatic lesions: 2.5cm segment 6, 1.8cm segment 8")
    .AddNote("New pulmonary nodules: bilateral, largest 1.2cm right lower lobe")
    .AddNote("RECIST 1.1 not applicable - no baseline target lesion measurements available")
    .Build();
```

#### Example: No Progression Detected

```csharp
var stableObservation = new RadiographicProgressionObservationBuilder(config)
    .WithInferenceId("rad-stable-001")
    .WithPatient("Patient/pt-456")
    .WithDevice("Device/ai-radiographic-v2")
    .WithFocus("Condition/lung-cancer")
    .WithNoProgressionDetected()
    .AddImagingStudy("ImagingStudy/ct-followup")
    .AddRadiologyReport("DocumentReference/report-456")
    .AddBodySite("39607008", "Lung structure")
    .WithImagingType("CT")
    .WithImagingDate(DateTime.UtcNow)
    .WithConfidence(0.88f)
    .WithQualitativeAssessment("Stable appearance of known pulmonary lesions without new lesions")
    .AddNote("No new lesions identified")
    .AddNote("Existing lesions appear stable in size and characteristics")
    .Build();
```
...
```csharp
// Core methods
public RadiographicProgressionObservationBuilder WithInferenceId(string id)
...
public RadiographicProgressionObservationBuilder WithConfidenceRationale(string rationale)
...
#### RECIST vs Radiographic Progression - When to Use Which

| Scenario | Use RECIST Builder | Use Radiographic Builder |
|----------|-------------------|--------------------------|
| Baseline target lesions measured | ✓ | |
| Follow-up with measurable disease | ✓ | |
| New lesions with RECIST baseline | ✓ | |
| No baseline measurements | | ✓ |
| Non-measurable disease only | | ✓ |
| Qualitative assessment needed | | ✓ |
| Bone-only metastases | | ✓ |
| Initial staging without baseline | | ✓ |
