...
public static class LangfuseQueryBuilder
{
        ...
        int? version = null)
    {
        ...
        if (fromCreatedAt.HasValue)
            queryParams.Add(
                $"fromCreatedAt={Uri.EscapeDataString(fromCreatedAt.Value.ToString("yyyy-MM-ddTHH:mm:ss.fffZ"))}");

        if (toCreatedAt.HasValue)
            queryParams.Add(
                $"toCreatedAt={Uri.EscapeDataString(toCreatedAt.Value.ToString("yyyy-MM-ddTHH:mm:ss.fffZ"))}");
        ...
        DateTime? toStartTime = null)
    {
        ...
        if (fromStartTime.HasValue)
            queryParams.Add(
                $"fromStartTime={Uri.EscapeDataString(fromStartTime.Value.ToString("yyyy-MM-ddTHH:mm:ss.fffZ"))}");

        if (toStartTime.HasValue)
            queryParams.Add(
                $"toStartTime={Uri.EscapeDataString(toStartTime.Value.ToString("yyyy-MM-ddTHH:mm:ss.fffZ"))}");
    ...
    LangfusePromptWithSchema : LangfusePromptResponse
{
    ...
    private string GenerateToolName()
    {
        ...
        string toolName = Name.ToLowerInvariant()
            .Replace("-", "_")
            .Replace(" ", "_");
    ...
    private string GenerateToolDescription()
    {
        if (!string.IsNullOrWhiteSpace(Prompt) && Prompt.Length <= 200)
            return
                $"Structured output tool for: {Prompt.Substring(0, Math.Min(200, Prompt.Length))}";
...
public static class LangfuseSchemaHelper
{
    ...
    {
        if (config == null || !config.TryGetValue("schema", out object? schemaValue))
            return null;
    ...
    {
        JsonDocument? schema = ExtractSchemaFromConfig(prompt.Config);
    ...
    /// </summary>
    private static string? ExtractToolNameFromConfig(Dictionary<string, object>? config,
        string promptName)
    {
        if (config?.TryGetValue("toolName", out object? toolNameValue) == true)
    ...
    /// </summary>
    private static string? ExtractToolDescriptionFromConfig(Dictionary<string, object>? config,
        string prompt)
    {
        if (config?.TryGetValue("toolDescription", out object? toolDescValue) == true)
...
public class BedrockTraceMetadata
{
    ...
    {
        get => Metadata.TryGetValue("documentId", out string? value) ? value : null;
        set
        {
            if (value != null) Metadata["documentId"] = value;
            else Metadata.Remove("documentId");
        }
    }

    public string? PracticeId
    {
        get => Metadata.TryGetValue("practiceId", out string? value) ? value : null;
        set
        {
            if (value != null) Metadata["practiceId"] = value;
            else Metadata.Remove("practiceId");
        }
    }

    public string? PatientId
    {
        get => Metadata.TryGetValue("patientId", out string? value) ? value : null;
        set
        {
            if (value != null) Metadata["patientId"] = value;
            else Metadata.Remove("patientId");
        }
    }

    public string? S3Bucket
    {
        get => Metadata.TryGetValue("s3Bucket", out string? value) ? value : null;
        set
        {
            if (value != null) Metadata["s3Bucket"] = value;
            else Metadata.Remove("s3Bucket");
        }
    }

    public string? S3Key
    {
        get => Metadata.TryGetValue("s3Key", out string? value) ? value : null;
        set
        {
            if (value != null) Metadata["s3Key"] = value;
            else Metadata.Remove("s3Key");
        }
    }
}
