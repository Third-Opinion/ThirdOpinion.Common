...
public static class BedrockTracingExtensions
{
        ...
        where TRequest : class
    {
        ...
        PropertyInfo? modelIdProp = requestType.GetProperty("ModelId");
        PropertyInfo? promptProp = requestType.GetProperty("Prompt");
        PropertyInfo? systemPromptProp = requestType.GetProperty("SystemPrompt");
        PropertyInfo? stopSequencesProp = requestType.GetProperty("StopSequences");
        PropertyInfo? maxTokensProp = requestType.GetProperty("MaxTokens");
        PropertyInfo? temperatureProp = requestType.GetProperty("Temperature");
        PropertyInfo? topPProp = requestType.GetProperty("TopP");
        PropertyInfo? parametersProp = requestType.GetProperty("Parameters");
        PropertyInfo? messagesProp = requestType.GetProperty("Messages");
        PropertyInfo? toolConfigProp = requestType.GetProperty("ToolConfiguration");

        string modelId = modelIdProp?.GetValue(request)?.ToString() ?? "unknown";
        metadata["modelId"] = modelId;

        var systemPrompt = systemPromptProp?.GetValue(request)?.ToString();
        if (!string.IsNullOrEmpty(systemPrompt)) metadata["systemPrompt"] = systemPrompt;

        var stopSequences = stopSequencesProp?.GetValue(request) as IEnumerable<string>;
        if (stopSequences?.Any() == true) metadata["stopSequences"] = stopSequences.ToArray();

        object? toolConfig = toolConfigProp?.GetValue(request);
        if (toolConfig != null)
            try
            {
                string toolConfigJson = JsonSerializer.Serialize(toolConfig);
                metadata["toolConfiguration"] = toolConfigJson;

                // Try to extract tool count if possible
                if (toolConfig.GetType().GetProperty("Tools")?.GetValue(toolConfig) is
                    IEnumerable<object> tools)
                {
                    List<object> toolsList = tools.ToList();
                    if (toolsList.Any()) metadata["toolCount"] = toolsList.Count;
                }
            }
            catch
            {
        ...
        object? maxTokens = maxTokensProp?.GetValue(request);
        if (maxTokens is int maxTokensInt)
            modelParameters["maxTokens"] = maxTokensInt;
        else if (maxTokens != null && int.TryParse(maxTokens.ToString(), out int parsedMaxTokens))
            modelParameters["maxTokens"] = parsedMaxTokens;

        object? temperature = temperatureProp?.GetValue(request);
        if (temperature is double tempDouble)
            modelParameters["temperature"] = tempDouble;
        else if (temperature != null &&
                 double.TryParse(temperature.ToString(), out double parsedTemp))
            modelParameters["temperature"] = parsedTemp;

        object? topP = topPProp?.GetValue(request);
        if (topP is double topPDouble)
            modelParameters["topP"] = topPDouble;
        else if (topP != null && double.TryParse(topP.ToString(), out double parsedTopP))
        ...
        if (messages?.Any() == true)
        {
            ...
            input = new
            {
                messages = messageArray, systemPrompt
            };
        }
        else
        {
            // Use simple prompt format
            string prompt = promptProp?.GetValue(request)?.ToString() ?? "";
            input = new
            {
                prompt, systemPrompt
            };
        ...
        where TResponse : class
    {
        ...
        PropertyInfo? contentProp = responseType.GetProperty("Content");
        PropertyInfo? usageProp = responseType.GetProperty("Usage");
        PropertyInfo? modelIdProp = responseType.GetProperty("ModelId");
        PropertyInfo? stopReasonProp = responseType.GetProperty("StopReason");
        PropertyInfo? requestIdProp = responseType.GetProperty("RequestId");
        PropertyInfo? durationProp = responseType.GetProperty("Duration");

        string content = contentProp?.GetValue(response)?.ToString() ?? "";
        generation.Output = new { content };

        // Set usage information
        object? usage = usageProp?.GetValue(response);
        if (usage != null)
        {
            object? inputTokens = usage.GetType().GetProperty("InputTokens")?.GetValue(usage);
            object? outputTokens = usage.GetType().GetProperty("OutputTokens")?.GetValue(usage);
            object? totalTokens = usage.GetType().GetProperty("TotalTokens")?.GetValue(usage);
        ...
        Dictionary<string, object> metadata
            = generation.Metadata ?? new Dictionary<string, object>();
        metadata["endTime"]
            = generation.EndTime?.ToString("yyyy-MM-ddTHH:mm:ss.fffZ") ?? string.Empty;

        object? duration = durationProp?.GetValue(response);
        ...
        if (!string.IsNullOrWhiteSpace(stopReason)) metadata["stopReason"] = stopReason;

        var requestId = requestIdProp?.GetValue(response)?.ToString();
        if (!string.IsNullOrWhiteSpace(requestId)) metadata["requestId"] = requestId;

        if (!string.IsNullOrWhiteSpace(region)) metadata["region"] = region;
        ...
        string? userId = null)
    {
        ...
        if (traceMetadata != null)
        {
            ...
            foreach (KeyValuePair<string, string> kvp in traceMetadata.Metadata)
        ...
        string? errorMessage = null)
    {
        ...
        Dictionary<string, object> metadata = trace.Metadata ?? new Dictionary<string, object>();
        metadata["completedAt"] = DateTime.UtcNow.ToString("yyyy-MM-ddTHH:mm:ss.fffZ");
        metadata["success"] = success;

        if (!string.IsNullOrWhiteSpace(errorMessage)) metadata["error"] = errorMessage;
    ...
    public static string ExtractModelName(string modelId)
    {
        ...
        {
            string modelPart = cleanId.Substring("anthropic.claude".Length);
            ...
            {
                string modelType = modelParts[0];
                string majorVersion = modelParts[1];
                string minorVersion = modelParts[2];

                if (int.TryParse(majorVersion, out _) && int.TryParse(minorVersion, out _))
                    return $"claude-{modelType}-{majorVersion}-{minorVersion}";

                if (int.TryParse(modelType, out _) && !int.TryParse(majorVersion, out _))
    ...
    public static string CreateSessionId(string? documentId = null, string? practiceId = null)
    {
        ...
        if (!string.IsNullOrWhiteSpace(documentId)) return documentId;

        if (!string.IsNullOrWhiteSpace(practiceId)) return practiceId;
    ...
    public static string CreateUserId(string? practiceId = null, string? patientId = null)
    {
        ...
        if (!string.IsNullOrWhiteSpace(patientId)) return patientId;
